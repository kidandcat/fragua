...
This is the fragua repository itself. Two audiences:

- **Working *on* fragua** (changing the Go code) — read this section, then
  [CONTRIBUTING.md](CONTRIBUTING.md), [VISION.md](VISION.md) and [ARCHITECTURE.md](ARCHITECTURE.md).
- **Using fragua to design a board** — skip to "## Fragua" below.

## Working on fragua itself

```sh
go test ./...                        # must be green
go vet ./...                         # must be clean
gofmt -l cmd internal agent          # must print nothing
go build -o fragua ./cmd/fragua
./fragua run                         # API + UI on 127.0.0.1:7878
```

Hard rules, from [VISION.md](VISION.md):

- **No external binaries at runtime.** No `kicad-cli`, no FreeRouting, no shell-out.
  Every format we read or write is implemented in Go here.
- **Agent-first.** The script API is the primary surface; the browser UI observes and
  steers the same in-memory project.
- Pure Go, one static binary. Generic Go modules are fine; CAD wrappers are not.

Layout (detail in [ARCHITECTURE.md](ARCHITECTURE.md)):

| Path | What |
|------|------|
| `internal/core` | project model, nm fixed-point geometry, JSON persistence |
| `internal/script` | the agent DSL — `dispatch.go` (verbs), `usage.go` (help table) |
| `internal/router` / `internal/placer` | auto-routing and placement |
| `internal/drc` / `internal/erc` / `internal/si` | checks |
| `internal/fab` / `internal/gerber` / `internal/odb` | manufacturing output |
| `internal/host` | HTTP API + embedded browser UI |
| `internal/mcp` | MCP server (stdio) over the same script API |
| `agent/` | the onboarding docs `fragua init` writes — **the product's front door** |
| `cmd/fragua` | CLI: `run`, `mcp`, `init`, `help` |

When you add or change a script verb, update **both** `internal/script/dispatch.go` and the
`Verbs` table in `internal/script/usage.go` (name, aliases, usage line, description,
examples). The help text, `GET /help`, `fragua help <verb>`, the MCP `fragua_help` tool and
`docs/llms-full.txt` all derive from that table. Regenerate the docs with
`scripts/gen-llms.sh`; a test fails if they drift.

Commits: `feat|fix|docs(scope): …`, lowercase, English.
