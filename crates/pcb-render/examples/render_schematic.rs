...
fn main() {
    ...
    let project = PathBuf::from(args.next().expect("usage: render_schematic <file.fragua> [out.png] [width]"));
    ...
    eprintln!("wrote {} ({} bytes)", out.display(), std::fs::metadata(&out).map(|m| m.len()).unwrap_or(0));
}
